...
Support for sfavit, moganet, maxvit, davit, 
...
parser.add_argument('--models', nargs='+',
                    choices=['sfavit', 'resnet50', 'maxvit', 'davit'],
                    default=['sfavit', 'resnet50', 'maxvit', 'davit'],
                    help="type models")
...
def append_models(args, device):
    ...
    if 'sfavit' in args.models:
        sfavit = timm.create_model('sfavit_t_224', pretrained=False).to(device).eval()
        state_dict = torch.load(args.checkpoint, weights_only=False)['state_dict']
        sfavit.load_state_dict(state_dict)
        _models.append(sfavit)
        models_layer.append(sfavit.stages[3].blocks[-1])
